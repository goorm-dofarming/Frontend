...
const Dropdown=({value,items,onClick}:{value:string, items:SortType[], onClick?:(item:SortType)=>void})=>{
    ...
    return(
        <DropdownContainer>
            <div ref={ref} onClick={onClickDropdown}   className={cx("dropdownInfo", { ["isOpen"]: isOpen })}>
                {value}
                {isOpen ? <IoIosArrowUp className={cx("icon", { ["isOpen"]: isOpen })}/> : <IoIosArrowDown className={cx("icon", { ["isOpen"]: isOpen })}/>}
            </div>
